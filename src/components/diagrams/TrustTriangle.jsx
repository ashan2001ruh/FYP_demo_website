  ...
  return (
    <div className="diagram-frame">
      <svg viewBox="0 0 720 430" role="img" aria-label="DID/VC trust triangle" style={{ minWidth: 520 }}>
        ...
        <NodeBox x={250} y={26} w={220} h={76} title="Issuer — the RIC" sub={['ACA-Py 0.10.4 + DIDKit key', 'KewdxLKBU9Fgu5aac8PH4R']} stroke="var(--accent)" />
        <NodeBox x={30} y={250} w={220} h={76} title="Holder — the xApp" sub={['wallet Secret /wallet', 'did:sov + did:key + signed VC']} stroke="var(--green)" />
        <NodeBox x={470} y={250} w={220} h={76} title="Verifier — Auth Agent" sub={['auth-agent:v2 sidecar', 'DIDKit VC + VP verification']} stroke="var(--purple)" />

        {/* ledger */}
        <g className="anim-pulse">
          <ellipse cx="360" cy="392" rx="150" ry="26" fill="rgba(247,120,186,0.08)" stroke="var(--pink)" strokeWidth="1.4" />
        </g>
        <text x="360" y="389" textAnchor="middle" className="svg-label" fontSize="13" fill="var(--pink)">Von Network · Hyperledger Indy</text>
        <text x="360" y="404" textAnchor="middle" className="svg-sub" fontSize="10.5">root of trust — anchors every DID</text>

        {/* issuer -> holder: issues VC */}
        <Wire d="M268 102 C190 150 160 200 148 250" stroke="var(--accent)" width={1.8} marker="url(#tt-arrow-teal)" />
        <WireLabel x={150} y={170}>issues signed VC</WireLabel>
        ...
        <WireLabel x={360} y={286}>presents VP · fresh challenge, every request</WireLabel>
        ...
        <WireLabel x={572} y={168}>trusts RIC issuer DID</WireLabel>
      ...
      <p className="diagram-caption">No identity server in the runtime path — trust is rooted in the ledger and Ed25519 signatures.</p>
    </div>
  )
}
